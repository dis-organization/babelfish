...
*  **1) dplyr-Spatial** is useful for when the limits of the `sp` Spatial classes are sufficient. This is pretty much mostly done in `spbabel` now, needs some fleshing out of the behaviour of outer joins and testing, but the left and inner join and group_by are pretty much working.  
...
There are shared vertices in this data set, so let's remove the duplication with a branch_vertex table. 
